...
public class ProductInfoDAO {
	...
	public List<ProductInfoDTO> getRelatedProductList(int categoryId, int productId, int limitOffset,int limitRowCount) {
		...
		/**
		 * round():表示順をランダムにする。
		 * limit 0,3 であれば、0番目から3件データを取得する
		 */
	...
	public List<ProductInfoDTO> getProductInfoListByKeyword(String[] keywordsList) {
		...
		Connection con = db.getConnection();
		List<ProductInfoDTO> productInfoDTOList = new ArrayList<ProductInfoDTO>();
		String sql = "select * from product_info"; // 下記条件によってselect文に追記するWHERE句が変わる

		boolean initializeFlag = true;	// 条件分岐用のフラグ

		if (!(keywordsList[0].equals(""))){ // 検索ワードが空じゃないときのみ実行
			for (String keyword : keywordsList) { // keywordsListは、別Actionクラスで検索ワードをスプリットして配列化している。
				if (initializeFlag) {
					sql += " where (product_name like '%" + keyword + "%' or product_name_kana like '%" + keyword + "%')";//ここの ' は出力する文字列
					// 2つ目の条件を追加する際(2ループ目以降)、whereではなくorと書く必要があるため、initializeFlagをfalseにする。
